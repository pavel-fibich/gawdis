...
author: "Francesco de Bello (fradebello@ext.uv.es)"
date: "9.10.2020"
...
### Loading functions
We will now open the package FD, which you should install on your computer, and the object "gawdis.R" which contains the function `gawdis`. Remember to have the file "gawdis.R" in your working directory!
...
### The Gower distance
We will start now by looking what the function does, overall. To do this let's first look at the original function `gawdis` with the data `dummy$trait`, which includes invented data for few species and their traits, available in the FD package (gawdis source the FD package).
...
```{r}
gaw.nogroups<-gawdis(tussock.trait.log, w.type = "optimized", opti.maxiter = 300)#there are NAs so the iteration approach is the only possible
...
```{r}
colnames(tussock.trait.log)
gaw.groups<-gawdis(tussock.trait.log, w.type = "optimized", opti.maxiter = 300, groups.weight=T, groups = c(1,2, 2, 2, 2, 2, 3, 4, 5, 6, 7))#there are NAs so the iteration approach is the only possible
...
We surely cannot apply `gowdis` to this type of matrices, because the function will "think" there is a total of 5 traits, since the matrix contains 5 columns, and will treat each of the 3 columns for colours as independent traits, resulting in a higher contribution of this single trait. Moreover the function gets a bit 'crazy' with this type of variables. Let's see why. We can use, to start with, the function only on the colours traits:
